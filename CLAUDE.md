# Component Library — Claude Code Master Plan
...
- Mark each phase complete in the checklist below before moving on.
...
### RTL test contract (non-negotiable)

Tests must only assert on:

- What is visible in the DOM (`getByRole`, `getByText`, `getByLabelText`)
- ARIA attributes (`aria-disabled`, `aria-busy`, `aria-label`)
- Events (click fires, click blocked when disabled)
- Accessibility (`axe` passes)

Tests must NEVER assert on:

- Internal state variable names
- Hook implementation details
- CSS class names (except via `toHaveClass` when class name is the API)

This contract ensures tests survive a future ui-core extraction unchanged.
...
## Appendix — Invariants Claude Code must never violate

These rules apply across all phases. If a task would violate one, stop and ask.
...
Never on internal state, hook names, or CSS class names (unless the class name
is the public API).
